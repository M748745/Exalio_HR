...
def main():
    ...
    # Run migrations on every app startup (lightweight, safe to run multiple times)
    if 'migrations_run' not in st.session_state:
        try:
            from database import apply_migrations, get_db_connection
            with get_db_connection() as conn:
                cursor = conn.cursor()
            ...
            pass  # Silently fail if migrations have issues
