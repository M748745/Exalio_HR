...
def show_workforce_analytics():
    ...
    with get_db_connection() as conn:
        ...
        cursor.execute("""
            SELECT
                CASE
                    WHEN julianday('now') - julianday(hire_date) < 365 THEN '< 1 year'
                    WHEN julianday('now') - julianday(hire_date) < 730 THEN '1-2 years'
                    WHEN julianday('now') - julianday(hire_date) < 1095 THEN '2-3 years'
                    WHEN julianday('now') - julianday(hire_date) < 1825 THEN '3-5 years'
                    ELSE '5+ years'
                END as tenure_group,
                COUNT(*) as count
            FROM employees
