...
def apply_migrations(cursor):
    ...
    try:
        ...
        """)
        notif_columns_info = {row['column_name']: row['data_type'] for row in cursor.fetchall()}

        if 'is_read' not in notif_columns_info:
        ...
        elif notif_columns_info.get('is_read') == 'integer':
            # Convert INTEGER to BOOLEAN
            cursor.execute("ALTER TABLE notifications ALTER COLUMN is_read TYPE BOOLEAN USING (is_read::int::boolean)")
            print("✅ Converted is_read column to BOOLEAN type")
